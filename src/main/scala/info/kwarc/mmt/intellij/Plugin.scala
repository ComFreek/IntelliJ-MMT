...
import info.kwarc.mmt.intellij.checking.ErrorViewerPanelWrapper
...
object MMT {
  ...
  def get(project: Project): Option[MMT] = {
    // TODO Do not call getService
    // 2019-03-08 15:38:40,541 [  50211]   WARN - napi.components.ServiceManager - info.kwarc.mmt.intellij.MMTProject requested as a service, but it is a component - convert it to a service or change call to project.getComponent()
    val mmtc = ServiceManager.getService(project, classOf[MMTProject])

    mmtc match {
      case null => None // why does this happen
      case _ if mmtc.isMMT => Some(mmtc.get)
      case _ => None
    }
  }
...
class MMT(val project: Project) {
  ...
  val mmtjarfile = File(PropertiesComponent.getInstance(project).getValue(MMTDataKeys.mmtjar))
  lazy val mmtjar = new MMTJar(mmtjarfile, this)

  lazy val errorViewer = new ErrorViewerPanelWrapper(project)
  lazy val shellViewer = new ShellViewerWrapper(project)
